...
export default function Quiz({params,}:{
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        let data: AxiosResponse = await axios.get(`https://turboquizapi.onrender.com/quiz/${params.id}`)
      ...
      </div>
      {
        ...
        loading ?
          <div className="flex justify-center">
            ...
            <div className="h-full bg-gray-600/45 backdrop-blur-xl md:w-1/2 rounded-lg shadow-xl  p-10 ">
          ...
          : <div>{!showResult ? (<div className="flex justify-center">
            <div className="h-full  md:w-1/2 rounded-lg shadow-xl bg-gray-600/45 backdrop-blur-xl p-10 ">
              ...
              <div className="h-24 md:flex mt-5 md:flex-wrap w-full justify-center">
                {options && options.map((ans, idx) => {
                  return (
                    <button onClick={() => onAnswerSelected(ans, idx)}
                      className={`border ${selectedAnswerIndex == idx ? 'bg-slate-400 bg-gray-600/45' : ''} hover:bg-slate-200 hover:text-black rounded-sm text-center w-full m-1 md:m-0 md:w-1/2`}
                      key={idx}
                    >
                      {ans}
                    </button>
            ...
            <div className="flex justify-center">
                ...
                <div className=" bg-gray-600/45 backdrop-blur-xl md:m-10 m-0 p-10 w-full md:w-1/2  rounded-xl text-white">
